...
# ============================================================
# CIC-IDS2017 -> ONE-CLASS SVM HYPERPARAMETER TUNING
# ============================================================
#
# Stage 18 = tune nu and gamma ONLY.
#
# Stage 17 has already fixed the anomaly-score threshold percentile.
# For each (nu, gamma) candidate, the threshold is recalculated from
# Validation BENIGN scores at that fixed percentile, exactly like the
# Isolation Forest hyperparameter-tuning stage keeps the threshold
# percentile fixed while rebuilding the threshold for each model.
#
...
BEST_MODEL_FILE = (
    BASE_DIR / "one_class_svm_behavior_best_validation.pkl"
)

RESULT_FILE = (
    BASE_DIR / "one_class_svm_hyperparameter_tuning_results.csv"
)

BEST_RESULT_FILE = (
    BASE_DIR / "one_class_svm_best_config.json"
)

SUMMARY_FILE = (
    BASE_DIR / "one_class_svm_hyperparameter_tuning_summary.txt"
)

VALIDATION_SCORE_FILE = (
    BASE_DIR / "one_class_svm_best_validation_scores.csv"
)

RANDOM_STATE = 42

# ------------------------------------------------------------
# One-Class SVM hyperparameter search space
# ------------------------------------------------------------
#
...
# ============================================================
# LOAD STAGE 17 THRESHOLD CONFIGURATION
# ============================================================
...
if not THRESHOLD_CONFIG_FILE.exists():
    ...
    raise FileNotFoundError(
        "ไม่พบ Stage 17 threshold config: "
        f"{THRESHOLD_CONFIG_FILE}"
    )
...
with open(
    ...
    stage17_threshold_config = json.load(f)

FIXED_THRESHOLD_PERCENTILE = float(
    stage17_threshold_config[
        "threshold_percentile"
    ]
)

if (
    FIXED_THRESHOLD_PERCENTILE < 0.0
    or FIXED_THRESHOLD_PERCENTILE > 100.0
):

    raise ValueError(
        "Stage 17 threshold percentile ไม่ถูกต้อง: "
        f"{FIXED_THRESHOLD_PERCENTILE}"
    )
...
print("=" * 80)
print("RUNNING ONE-CLASS SVM HYPERPARAMETER SEARCH")
print("=" * 80)

print(
    f"Total OCSVM configurations: {total_models}"
)

print(
    f"Fixed threshold percentile: "
    f"{FIXED_THRESHOLD_PERCENTILE:.0f}"
)
...
for nu_value in NU_VALUES:
    ...
    for gamma_value in GAMMA_VALUES:
        ...
        print("-" * 80)
        print(
            f"Configuration {search_counter}/{total_models}"
        )
        print(
            f"nu    = {nu_value}"
        )
        ...
        if len(validation_benign_scores) == 0:
            ...
            raise RuntimeError(
                "Validation ไม่มี BENIGN samples"
            )
        ...
        row = {
            ...
            "threshold_percentile": FIXED_THRESHOLD_PERCENTILE,
            "threshold": threshold,
        ...
        print(
            f"Validation Recall     : {metrics['recall']:.4f}"
        )
        print(
            f"Validation F1         : {metrics['f1']:.4f}"
        )
        print(
            f"Validation FPR        : {metrics['fpr']:.4f}"
        )
        print()
...
if results_df.empty:
    ...
    raise RuntimeError(
        "ไม่พบผลลัพธ์จาก One-Class SVM Hyperparameter Tuning"
    )
...
print()
print("=" * 80)
print("BEST ONE-CLASS SVM CONFIGURATION")
print("=" * 80)
...
with open(
    ...
    f.write(
        "Stage 18 = One-Class SVM Hyperparameter Tuning\n\n"
    )
    ...
    f.write(
        "Stage 17 selected the threshold percentile; Stage 18 keeps that percentile fixed for every hyperparameter candidate.\n"
    )
    ...
    f.write(
        "Hyperparameter and threshold selection uses Validation only.\n"
    )
    ...
    f.write(
        "The best configuration is saved for the later Final OCSVM stage.\n"
    )
...
print("=" * 80)
print("ONE-CLASS SVM HYPERPARAMETER TUNING COMPLETE")
print("=" * 80)

print(
    f"Tuning result file : {RESULT_FILE}"
)

print(
    f"Best model file    : {BEST_MODEL_FILE}"
)

print(
    f"Best config file   : {BEST_RESULT_FILE}"
)

print(
    f"Best score file    : {VALIDATION_SCORE_FILE}"
)
...
print(
    "Ready for Final One-Class SVM model training."
)
